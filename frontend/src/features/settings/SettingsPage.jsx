    ...
    <div className="bg-surface rounded-card border border-ink-100 overflow-hidden">
      {modal && (
        <SimpleModal
          ...
          defaultValues={modal.item ?? { soNgayGuiTruoc: 5, isActive: true }}
          fields={fields}
          onClose={() => setModal(null)}
          onSubmit={async (data) => {
            const payload = { ...data, thang: Number(data.thang), ngay: Number(data.ngay), soNgayGuiTruoc: Number(data.soNgayGuiTruoc), apDungChoLoaiKH: "TatCa" };
