...
export default function CreatePhieuThuModal({
  ...
  const handleSubmit = async (e) => {
    ...
    if (soTienConLai != null && amount > soTienConLai) {
      setError(`Số tiền thu không được vượt quá số còn lại (${formatCurrency(soTienConLai)})`);
      return;
    }
        ...
        <div className="text-sm bg-surface-alt rounded-lg p-3 space-y-1">
          ...
          {soTienConLai != null && (
            <div className="flex justify-between">
              ...
              <span className="font-medium text-danger-600">{formatCurrency(soTienConLai)}</span>
            </div>
          )}
        </div>
