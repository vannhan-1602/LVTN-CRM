...
export default function InvoiceDetailPage() {
  ...
  const {
    ...
    // Tự tải lại — bắt trạng thái thanh toán vừa được kế toán khác ghi phiếu thu.
    refetchInterval: 5000,
    refetchOnWindowFocus: true,
  });

  const invoice = data?.invoice ?? null;
  const phieuList = data?.phieuList ?? [];

  if (loading)
    return <div className="text-center py-20 text-ink-400">Đang tải...</div>;
  if (error)
    return <div className="text-danger-600 text-center py-20">{error}</div>;
  ...
  const pctDaThu =
    invoice.tongTien > 0
      ? Math.min(
          100,
          Math.round((invoice.soTienDaThu / invoice.tongTien) * 100),
        )
      : 0;
  ...
  return (
    <div className="space-y-5 max-w-4xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <Badge
          label={
            STATUS_LABEL[invoice.trangThaiThanhToan] ??
            invoice.trangThaiThanhToan
          }
          colorClass={
            STATUS_COLOR[invoice.trangThaiThanhToan] ??
        ...
        <div className="flex gap-6">
          ...
          <div>
            ...
            <p className="text-lg font-bold text-success-600">
              {formatCurrency(invoice.soTienDaThu)}
            </p>
          </div>
          <div>
            <p className="text-xs text-ink-400">Còn lại</p>
            <p className="text-lg font-bold text-danger-600">
              {formatCurrency(invoice.soTienConLai)}
            </p>
          </div>
        </div>
        ...
        <div className="px-5 py-3.5 border-b border-ink-100 flex items-center justify-between">
          ...
          {canAddPhieuThu && invoice.trangThaiThanhToan !== "HoanTat" && (
            <Button
              size="sm"
              icon={Plus}
              onClick={() => setShowPhieuThuModal(true)}
            >
              Tạo phiếu thu
            </Button>
          )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-surface-alt">
                {["Mã phiếu", "Loại", "Số tiền", "Người lập", "Ngày tạo"].map(
                  (h) => (
                    <th
                      key={h}
            ...
            <tbody className="divide-y divide-ink-100">
              {phieuList.map((p) => (
                ...
                >
                  <td className="px-5 py-3 font-mono text-info-600 text-xs font-semibold">
                    {p.maPhieu}
                  </td>
