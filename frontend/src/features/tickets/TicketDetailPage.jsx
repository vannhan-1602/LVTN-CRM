...
export default function TicketDetailPage() {
  ...
  const [phanHoiForm, setPhanHoiForm] = useState({ loaiPhanHoi: "NoiBoXuLy", noiDung: "" });
  ...
  const handleAddPhanHoi = async (e) => {
    ...
    setSubmittingPhanHoi(true); setError("");
    try {
  ...
  if (error && !ticket) {
    return (
      <div className="space-y-4">
        <PageHeader breadcrumb="CRM / Hỗ trợ" title="Ticket" onBack={() => navigate("/tickets")} />
      ...
      <PageHeader
        ...
        badge={
          <div className="flex gap-1.5">
            <Badge label={TICKET_STATUS[ticket.trangThai] ?? ticket.trangThai} colorClass={TICKET_STATUS_COLOR[ticket.trangThai]} />
            <Badge label={TICKET_PRIORITY[ticket.mucDoUuTien] ?? ticket.mucDoUuTien} colorClass={TICKET_PRIORITY_COLOR[ticket.mucDoUuTien]} />
          </div>
        }
        actions={
          <>
            {!isClosed && <Button variant="secondary" icon={Lock} onClick={handleClose}>Đóng ticket</Button>}
        ...
        <div className="lg:col-span-2 space-y-4">
          <Card title="Thông tin ticket">
            <div className="grid grid-cols-2 gap-5 mb-4">
              ...
              {ticket.ngayDong && <Field label="Ngày đóng" value={formatDateTime(ticket.ngayDong)} />}
              {ticket.lyDoDong && <Field label="Lý do đóng" value={ticket.lyDoDong} />}
            </div>
            {ticket.moTa && (
          ...
          <Card title={`Lịch sử phản hồi (${ticket.phanHois?.length ?? 0})`}>
            {(!ticket.phanHois || ticket.phanHois.length === 0) ? (
              ...
              <div className="space-y-2.5 max-h-80 overflow-y-auto">
                {ticket.phanHois.map((ph) => (
                  <div key={ph.id} className="bg-surface-alt rounded-lg p-3 text-sm">
                    <div className="flex items-center justify-between mb-1 gap-2">
                      <span className="font-medium text-ink-900">
                        {nhanVienMap.get(String(ph.nguoiPhanHoiId)) ?? `NV #${ph.nguoiPhanHoiId}`}
                        <span className="text-xs text-ink-400 font-normal ml-1.5">
                          — {TICKET_PHAN_HOI_LABEL[ph.loaiPhanHoi] ?? ph.loaiPhanHoi}
                        </span>
                      </span>
                      <span className="text-xs text-ink-400 shrink-0">{formatDateTime(ph.createdAt)}</span>
                    </div>
                    <p className="text-ink-700">{ph.noiDung}</p>
                    {(ph.trangThaiTruoc || ph.trangThaiSau) && (
                      <p className="text-xs text-ink-400 mt-1">
                        {TICKET_STATUS[ph.trangThaiTruoc] ?? ph.trangThaiTruoc} → {TICKET_STATUS[ph.trangThaiSau] ?? ph.trangThaiSau}
                      </p>
                    )}
                  </div>
                ))}
            ...
            {!isClosed && (
              <form onSubmit={handleAddPhanHoi} className="border-t border-ink-100 pt-4 mt-4 space-y-3">
                <h4 className="text-sm font-medium text-ink-900">Thêm phản hồi</h4>
                <select value={phanHoiForm.loaiPhanHoi} onChange={(e) => setPhanHoiForm((f) => ({ ...f, loaiPhanHoi: e.target.value }))}
                  ...
                  {TICKET_PHAN_HOI_LOAI_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
                <textarea value={phanHoiForm.noiDung} onChange={(e) => setPhanHoiForm((f) => ({ ...f, noiDung: e.target.value }))}
                  placeholder="Nội dung phản hồi..." rows={3}
                ...
                <Button type="submit" size="sm" icon={Send} disabled={submittingPhanHoi || !phanHoiForm.noiDung.trim()}>
        ...
        <div className="space-y-4">
          <Card title="Cập nhật xử lý">
            <form onSubmit={handleSaveEdit} className="space-y-3">
              <div>
                <label className="block text-xs font-medium text-ink-500 mb-1">Trạng thái</label>
              ...
              <div>
                <label className="block text-xs font-medium text-ink-500 mb-1">Mức ưu tiên</label>
                <select value={editForm.mucDoUuTien} onChange={(e) => setEditForm((f) => ({ ...f, mucDoUuTien: e.target.value }))}
                  disabled={isClosed}
                  className="w-full border border-ink-200 rounded-lg px-3 py-2 text-sm disabled:bg-ink-100 disabled:text-ink-400">
              ...
              <div>
                <label className="block text-xs font-medium text-ink-500 mb-1">Ngày hẹn xử lý</label>
                <input type="datetime-local" value={editForm.ngayHenXuLy}
                  onChange={(e) => setEditForm((f) => ({ ...f, ngayHenXuLy: e.target.value }))}
                  disabled={isClosed}
                  className="w-full border border-ink-200 rounded-lg px-3 py-2 text-sm disabled:bg-ink-100 disabled:text-ink-400" />
              </div>
              {!isClosed && (
              ...
              {isClosed && <p className="text-xs text-ink-400">Ticket đã đóng, không thể chỉnh sửa thêm.</p>}
            </form>
          </Card>
