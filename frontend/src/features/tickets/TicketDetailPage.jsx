...
export default function TicketDetailPage() {
  ...
  const handleSaveEdit = async (e) => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Cập nhật ticket thất bại"));
    } finally {
      setSavingEdit(false);
    }
  };
  ...
  const handleAssign = async (e) => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Gán nhân viên thất bại"));
    } finally {
  ...
  const handleAddPhanHoi = async (e) => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Thêm phản hồi thất bại"));
    } finally {
  ...
  const handleClose = async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Không thể đóng ticket"));
    }
  };
        ...
        <div className="lg:col-span-2 space-y-4">
          <Card title="Thông tin ticket">
            <div className="grid grid-cols-2 gap-5 mb-4">
              ...
              <Field
                label="Khách hàng"
                value={ticket.tenKhachHang ?? `KH #${ticket.khachHangId}`}
              />
              {ticket.soDienThoaiKhachHang && (
                <Field
                  label="Số điện thoại"
                  value={ticket.soDienThoaiKhachHang}
                />
              )}
              {ticket.emailKhachHang && (
                <Field label="Email" value={ticket.emailKhachHang} />
              )}
              {ticket.ngayDong && (
                <Field
                  label="Ngày đóng"
              ...
              {user?.role === ROLES.Sale ? (
                ...
                ticket.nhanVienXuLyId === user?.userId ? (
                  <p className="text-sm text-ink-700">
                    Bạn đang xử lý ticket này. Liên hệ Manager nếu cần chuyển
                    cho người khác.
                  </p>
                ) : (
                  <Button
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-1">
                    {[1, 2, 3, 4, 5].map((n) => (
                      <span
                        key={n}
                        className={
                          n <= ticket.csat.diemDanhGia
                            ? "text-purple-600 text-lg leading-none"
                            : "text-purple-200 text-lg leading-none"
                        }
                      >
                        ★
                      </span>
                  ...
                  {ticket.csat.ngayDanhGia && (
                    <p className="text-xs text-purple-500">
                      Khách đánh giá lúc{" "}
                      {formatDateTime(ticket.csat.ngayDanhGia)}
                    </p>
                  )}
                </div>
              ) : (
