...
import { TICKET_PRIORITY_OPTIONS, TICKET_SOURCE_OPTIONS } from "../../utils/constants";
...
export default function CreateTicketModal({ customers, nhanVienList, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await ticketApi.create({
        ...
        nhanVienXuLyId: form.nhanVienXuLyId ? Number(form.nhanVienXuLyId) : null,
        ngayHenXuLy: form.ngayHenXuLy || null,
      });
      onSaved();
    } catch (err) { setError(err?.message || "Không thể tạo ticket"); }
    ...
    <Modal isOpen onClose={onClose} title="Tạo ticket hỗ trợ mới" size="md">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input value={form.tieuDe} onChange={(e) => set("tieuDe", e.target.value)}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-sm font-medium text-ink-700 mb-1.5">Mức ưu tiên</label>
            <select value={form.mucDoUuTien} onChange={(e) => set("mucDoUuTien", e.target.value)}
              className="w-full border border-ink-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent-400/40 focus:border-accent-400">
              {TICKET_PRIORITY_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-ink-700 mb-1.5">Nguồn tiếp nhận</label>
            <select value={form.nguonTiepNhan} onChange={(e) => set("nguonTiepNhan", e.target.value)}
              className="w-full border border-ink-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent-400/40 focus:border-accent-400">
              {TICKET_SOURCE_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
            </select>
          </div>
        </div>

        <div>
          <label className="block text-sm font-medium text-ink-700 mb-1.5">Nhân viên xử lý</label>
          <select value={form.nhanVienXuLyId} onChange={(e) => set("nhanVienXuLyId", e.target.value)}
            className="w-full border border-ink-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent-400/40 focus:border-accent-400">
        ...
        <div>
          <label className="block text-sm font-medium text-ink-700 mb-1.5">Ngày hẹn xử lý</label>
          <input type="datetime-local" value={form.ngayHenXuLy} onChange={(e) => set("ngayHenXuLy", e.target.value)}
            className="w-full border border-ink-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent-400/40 focus:border-accent-400" />
        </div>

        <div>
          <label className="block text-sm font-medium text-ink-700 mb-1.5">Mô tả chi tiết</label>
