...
export default function OpportunityFormModal({ item, customers, leads, nhanVienList = [], canAssign = false, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      nhanVienPhuTrachId: form.nhanVienPhuTrachId ? Number(form.nhanVienPhuTrachId) : null,
    };
    try {
    ...
    <Modal isOpen onClose={onClose} title={isEdit ? `Sửa cơ hội: ${item.tenThuongVu}` : "Tạo cơ hội bán hàng mới"} size="md">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          {canAssign ? (
            <select value={form.nhanVienPhuTrachId} onChange={(e) => set("nhanVienPhuTrachId", e.target.value)}
              className="w-full border border-ink-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent-400/40 focus:border-accent-400">
