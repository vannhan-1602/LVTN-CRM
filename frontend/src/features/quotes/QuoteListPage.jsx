...
export default function QuoteListPage() {
  ...
  const loadLookups = async () => {
    try {
      ...
      /* không có quyền hoặc lỗi tải dữ liệu tham chiếu, bỏ qua */
    }
  };
    ...
    <div className="space-y-5">
      {formModal && (
        <QuoteFormModal
          ...
          onSaved={() => {
            setFormModal(null);
            setSuccess("Lưu báo giá thành công");
            loadQuotes();
          }}
        />
      )}
      ...
      <PageHeader
        ...
        actions={
          canEdit && (
            <Button icon={Plus} onClick={() => setFormModal({})}>
              Lập báo giá
            </Button>
          )
        }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <StatCard
          label="Tổng báo giá (trang này)"
          value={stats.total}
          icon={Receipt}
        />
        <StatCard
          label="Đã chấp nhận"
          value={stats.chapNhan}
          tone="success"
          icon={CheckCircle2}
        />
        <StatCard
          label="Tổng giá trị (trang này)"
          value={formatMoney(stats.tongTien)}
          tone="accent"
          icon={Wallet}
        />
      </div>
              ...
              {items.length === 0 ? (
                <tr>
                  <td colSpan={6}>
                    <EmptyState
                      icon={Receipt}
                      title={loading ? "Đang tải..." : "Chưa có báo giá nào"}
                      description={
                        !loading && canEdit
                          ? "Lập báo giá mới cho khách hàng."
                          : undefined
                      }
                    />
                  </td>
                ...
                items.map((item) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5">
                      <Badge
                        label={QUOTE_STATUS[item.trangThai] ?? item.trangThai}
                        colorClass={QUOTE_STATUS_COLOR[item.trangThai]}
                      />
                    </td>
                            ...
                            ? [
                                {
                                  label: "Sửa",
                                  icon: Pencil,
                                  onClick: () => setFormModal({ quote: item }),
                                },
                              ]
                            : []),
