...
export default function ProductListPage() {
  ...
  const loadTypes = async () => {
    try {
      ...
      /* không có quyền hoặc lỗi tải loại sản phẩm, bỏ qua */
    }
  };
  ...
  const handleReactivate = async (item) => {
    if (
      !window.confirm(
        "Mở lại kinh doanh sản phẩm này? Sản phẩm sẽ xuất hiện lại khi lập báo giá mới.",
      )
    )
      return;
    try {
              ...
              {items.length === 0 ? (
                <tr>
                  <td colSpan={8}>
                    <EmptyState
                      icon={Package}
                      title={loading ? "Đang tải..." : "Chưa có sản phẩm nào"}
                      description={
                        !loading && canManage
                          ? "Thêm sản phẩm mới để bắt đầu lập báo giá."
                          : undefined
                      }
                    />
                  </td>
                    ...
                    <td className="px-5 py-3.5">
                      {item.anhDaiDien ? (
                        <img
                          ...
                          alt={item.tenSP}
                          className="w-10 h-10 rounded-lg object-cover border border-ink-100"
                        />
                      ) : (
                        <div className="w-10 h-10 rounded-lg bg-surface-alt border border-ink-100 flex items-center justify-center">
