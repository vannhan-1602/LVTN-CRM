...
export default function UserManagementPage() {
  ...
  // Backend báo có user mới/sửa/xóa/khóa -> tự tải lại danh sách.
  ...
  const handleToggleStatus = async (user) => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Không thể đổi trạng thái"));
    }
  };
  ...
  const handleDelete = async (user) => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Không thể xóa tài khoản"));
    }
  };
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={7}>
                    <EmptyState
                      icon={Users}
                      title={loading ? "Đang tải..." : "Không có dữ liệu"}
                    />
                  </td>
                </tr>
              ) : (
                filtered.map((u) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5">
                      <Badge
                        label={u.roleName ?? "—"}
                        colorClass={USER_ROLE_COLOR[u.roleName]}
                      />
                    </td>
                    <td className="px-5 py-3.5">
                      <Badge
                        label={USER_STATUS_LABEL[u.trangThai] ?? u.trangThai}
                        colorClass={USER_STATUS_COLOR[u.trangThai]}
                      />
                    </td>
                    <td
