...
import { ROLES, LOAI_BADGE_COLOR, TINH_TRANG_BADGE_COLOR } from "../../utils/constants";
...
function AddressSection({ customerId, canEdit }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.diaChiChiTiet.trim()) { setError("Địa chỉ chi tiết không được để trống"); return; }
    ...
    try {
      ...
      await load();
      resetForm();
    } catch (err) { setError(err?.message || "Không thể lưu địa chỉ"); }
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="border border-ink-100 rounded-lg p-4 bg-surface-alt space-y-3 mb-4">
          <div className="flex items-center justify-between">
            <h4 className="text-sm font-medium text-ink-900">{editingId ? "Sửa địa chỉ" : "Thêm địa chỉ mới"}</h4>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-ink-500 mb-1">Loại địa chỉ</label>
              <select value={form.loaiDiaChi} onChange={(e) => setForm((f) => ({ ...f, loaiDiaChi: e.target.value }))}
                className="w-full border border-ink-200 rounded-lg px-3 py-2 text-sm">
                {LOAI_DIA_CHI_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
            </div>
          ...
          <div>
            ...
            <input value={form.diaChiChiTiet} onChange={(e) => setForm((f) => ({ ...f, diaChiChiTiet: e.target.value }))}
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-medium text-ink-500 mb-1">Phường/Xã</label>
              <input value={form.phuongXa} onChange={(e) => setForm((f) => ({ ...f, phuongXa: e.target.value }))}
                placeholder="Phường/Xã" className="w-full border border-ink-200 rounded-lg px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-xs font-medium text-ink-500 mb-1">Quận/Huyện</label>
              <input value={form.quanHuyen} onChange={(e) => setForm((f) => ({ ...f, quanHuyen: e.target.value }))}
                placeholder="Quận/Huyện" className="w-full border border-ink-200 rounded-lg px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-xs font-medium text-ink-500 mb-1">Tỉnh/Thành phố</label>
              <input value={form.tinhThanh} onChange={(e) => setForm((f) => ({ ...f, tinhThanh: e.target.value }))}
                placeholder="Tỉnh/Thành phố" className="w-full border border-ink-200 rounded-lg px-3 py-2 text-sm" />
            </div>
          </div>
          ...
          <div className="flex gap-2">
            ...
            <Button type="button" size="sm" variant="secondary" onClick={resetForm}>Hủy</Button>
          </div>
        </form>
      )}
            ...
            <div key={addr.id}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  <Badge label={LOAI_DIA_CHI_OPTIONS.find((o) => o.value === addr.loaiDiaChi)?.label ?? addr.loaiDiaChi} tone="info" />
                  {addr.isDefault && (
                    <span className="inline-flex items-center gap-1 text-xs text-accent-700 font-medium">
                      <Star size={12} className="fill-accent-500 text-accent-500" /> Mặc định
                    </span>
                  )}
                </div>
              ...
              {canEdit && (
                <div className="flex gap-3 shrink-0">
                  <button onClick={() => handleEdit(addr)} className="text-xs font-medium text-info-600 hover:underline">Sửa</button>
                  <button onClick={() => handleDelete(addr.id)} className="text-xs font-medium text-danger-600 hover:underline">Xóa</button>
                </div>
              )}
            </div>
          ))}
...
export default function CustomerDetailPage() {
  ...
  const load = async () => {
    ...
    } catch (err) { setError(err?.message || "Không thể tải thông tin khách hàng"); }
    finally { setLoading(false); }
  };
  ...
  if (error || !customer) {
    return (
      <div className="space-y-4">
        <PageHeader breadcrumb="CRM / Khách hàng" title="Khách hàng" onBack={() => navigate("/customers")} />
        <div className="text-sm text-danger-600 bg-danger-50 rounded-lg p-4">{error || "Không tìm thấy khách hàng."}</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <PageHeader
        ...
        badge={customer.tinhTrangId && (
          <Badge label={customer.tenTinhTrang ?? `Tình trạng ${customer.tinhTrangId}`}
            colorClass={TINH_TRANG_BADGE_COLOR[customer.tinhTrangId]} />
        )}
        actions={
          <>
            {canEdit && <Button variant="secondary" icon={Pencil} onClick={() => setShowEditModal(true)}>Sửa</Button>}
        ...
        <div className="lg:col-span-2 space-y-4">
          <Card title="Thông tin khách hàng">
            <div className="grid grid-cols-2 gap-5">
              <Field label="Mã khách hàng" value={<span className="font-mono">{customer.maKhachHang}</span>} />
              <Field label="Loại khách hàng" value={
                customer.loaiKhachHangId
                  ? <Badge label={customer.tenLoaiKhachHang ?? `Loại ${customer.loaiKhachHangId}`} colorClass={LOAI_BADGE_COLOR[customer.loaiKhachHangId]} />
                  : "—"
              } />
              <Field label="Email" value={customer.email} />
              <Field label="Số điện thoại" value={customer.soDienThoai} />
              <Field label="Mã số thuế" value={customer.maSoThue} />
              <Field label="Nhân viên phụ trách" value={customer.tenNhanVienPhuTrach} />
              <Field label="Ngày tạo" value={formatDateTime(customer.createdAt)} />
              <Field label="Cập nhật gần nhất" value={formatDateTime(customer.updatedAt)} />
            </div>
          </Card>
          ...
          <Card title="Liên kết nhanh">
            <div className="space-y-2">
              <Button variant="secondary" size="sm" className="w-full"
                onClick={() => navigate(`/opportunities?khachHangId=${customer.id}`)}>
                Cơ hội bán hàng
              </Button>
              <Button variant="secondary" size="sm" className="w-full"
              ...
              <Button variant="secondary" size="sm" className="w-full"
                onClick={() => navigate(`/contracts?khachHangId=${customer.id}`)}>
                Hợp đồng
              </Button>
              <Button variant="secondary" size="sm" className="w-full"
